...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase = 'train',split_comber=None):
        ...
        idcs = [str(f)[2:-1] for f in idcs if (f not in self.blacklist)]
        if phase!='test':
            idcs = [f for f in idcs if (f not in self.blacklist)]
        idcs = [f for f in idcs if os.path.exists(data_dir + f"/{f}_label.npy")]

        print(len(idcs))
        self.filenames = [data_dir+f"/{idx}_clean.npy" for idx in idcs]
        ...
        for idx in idcs:
            l = np.load(data_dir + f"/{idx}_label.npy")
            if np.all(l==0):
                l=np.array([])
            labels.append(l)
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            if not isRandomImg:
                ...
                for i in range(3):
                    if sample.shape[1:][i] % 4 != 0:
                        print('if ', sample.shape, sample.shape[1:][i] % 4)
                if self.phase=='train' and not isRandom:
                    ...
                    for i in range(3):
                        if sample.shape[1:][i] % 4 != 0:
                            print('if if', sample.shape, sample.shape[1:][i] % 4)
                ...
                for i in range(3):
                    if sample.shape[1:][i] % 4 != 0:
                        print('else ', sample.shape, sample.shape[1:][i] % 4)
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        # print(crop.shape)
        if crop.shape[-1] != 128:
            print('Crop', imgs.shape, crop.shape, before_scale_shape, 'pad:',pad, 'start:',start, 'crop size:', crop_size)
        return crop, target, bboxes, coord
    
class LabelMapping(object):
    def __init__(self, config, phase):
        self.stride = np.array(config['stride'], dtype=np.int)
    ...
    def __call__(self, input_size, target, bboxes):
        ...
        for i in range(3):
            if input_size[i] % stride != 0:
                print(input_size, stride, input_size[i] % stride)
